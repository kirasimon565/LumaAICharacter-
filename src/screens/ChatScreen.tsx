...
const ChatScreen = () => {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      try {
        ...
        // If no history, add greeting from character locally
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const systemPrompt = `
        You are ${char?.name || 'an AI'}. 
        Personality: ${char?.personality || ''}
        Backstory: ${char?.backstory || ''}
        Writing Style: ${char?.style || ''}
        Instructions: ${char?.instructions || ''}
        Stay in character. Do not break the fourth wall.
      `;

      // Construct history string
      const context = messages.slice(-10).map(m => `${m.sender === Sender.USER ? 'User' : (char?.name || 'AI')}: ${m.text}`).join('\n');
  ...
  const characterImage = chat?.expand?.character?.Image;
  const characterName = chat?.expand?.character?.name || 'Unknown';
  ...
  return (
    <div className="h-screen flex flex-col bg-black">
      ...
      <div className="h-16 flex items-center px-4 border-b border-white/10 bg-black/80 backdrop-blur-md z-10">
        ...
        <img 
          src={getFileUrl(chat?.expand?.character?.id || '', 'characters', characterImage || '')} 
          className="w-10 h-10 rounded-full object-cover border border-[#F5D48B]/50"
          alt="char"
        />
        <div className="ml-3">
          <h3 className="text-white font-serif">{characterName}</h3>
